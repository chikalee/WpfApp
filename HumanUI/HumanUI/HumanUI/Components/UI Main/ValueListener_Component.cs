    ...
    public class ValueListener_Component : GH_Component, IGH_VariableParameterComponent
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
           ...
           if(WindowInfo.SharedWindow != null) {
            ...
            else   HUI_Util.extractBaseElements(filteredElements, elementsToListen);
            ...
            {
                object value=null, indices=null;
                if (WindowInfo.SharedWindow != null)
                {
                    WindowInfo.SharedWindow.Invoke(new Action(() =>
                    {

                        value = HUI_Util.GetElementValue(u);
                        indices = HUI_Util.GetElementIndex(u);
                    }));
                }
                else
                {
        ...
        void AddEvents(UIElement u)
        {
            ...
            switch (u.GetType().ToString())
            {
                ...
                case "Xceed.Wpf.Toolkit.ColorPicker":
                    Xceed.Wpf.Toolkit.ColorPicker cp = u as Xceed.Wpf.Toolkit.ColorPicker;
        ...
        void ExpireThis(object sender, EventArgs e)
        {
            if (WindowInfo.SharedWindow != null)
            {
                ...
                WindowInfo.SharedWindow.expireToken = true;
                if (WindowInfo.SharedWindow.allowToExpire && !waiting) WindowInfo.SharedWindow.expireAction.Add(() => { this.ExpireSolution(false); waiting = false; });
                waiting = true;
            }
            else   this.ExpireSolution(true);
